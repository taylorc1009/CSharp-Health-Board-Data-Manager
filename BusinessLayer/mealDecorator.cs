...
{
    //Decorates visit type 3
    class mealDecorator : visitDecorator
    {
        public override void validateType(Dictionary<int, Staff> members, int[] staff, int type)
        {
            ...
            else if (members[staff[0]].category != "Care Worker")
        ...
        public override void validateTime(List<Visit> visits, int[] staff, int patient, DateTime dateTime)
        {
            ...
            {
                //Stores the time span of the current visit based on its type, for "meal" this is between the start time + 30 minutes
